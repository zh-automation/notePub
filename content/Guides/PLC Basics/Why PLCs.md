...
### Memory
One of the concepts that is key to controlling end devices, or reading data from them, is how does the data get to and from the CPU. There's a whole lot of steps between them, but the end result is facilitated through the use of memory-mapped I/O (MMIO). 
